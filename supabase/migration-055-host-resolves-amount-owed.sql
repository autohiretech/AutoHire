-- AutoHire migration 055 — a host can resolve what's owed themselves.
--
-- `amount_owed_rwf` (migration 053) is computed by payhold-settle-usage and
-- was locked like every other amount column — nothing could ever change it.
-- That was right while it was purely informational, but a host now needs to
-- say what actually happened: they collected it themselves (mark it 0), or
-- they're waiving some or all of it (lower it). PayHold is not involved
-- either way — there is still no way to charge a renter again once a deal
-- has funded, and this migration doesn't try to build one. It only lets the
-- host record the outcome.
--
-- The guard is narrow on purpose: only the host, and only downward. A host
-- raising what's owed would be inventing a charge with no deal behind it;
-- lowering it is just them saying less is actually outstanding than the
-- system computed.
--
-- Safe to re-run.
