...
| `…075_search_distance` | Expands `search_available_listings()` (not a second function) with optional `p_near_lat`/`p_near_lng`, appended so the old 11-param signature still works. When given, orders by real haversine distance (km, `acos` argument clamped into `[-1, 1]` against float rounding) with any listing missing lat/lng sorted last, never excluded; when omitted, the distance expression is `null` for every row and the order is byte-for-byte the same `rating_avg desc, id asc` as before |
| `…076_search_keyword_punctuation` | **Written, not yet pushed.** `search_available_listings()`'s keyword AND used to require a word made only of stripped characters (`%*,()`) to *fail*, mirroring the client forcing `id.eq.__no_match__` — so a lone `,` from a geocoded address zeroed the whole search. Strips each word first and drops the ones left empty, so punctuation narrows nothing instead of excluding everything. Same 13-param signature as 075 (a plain `create or replace`, no drop); every filter, the availability `not exists` and the haversine `order by` are copied from 075 unchanged |
